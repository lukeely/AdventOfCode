...
object day24 extends Day[(Map[String, Boolean], Seq[Gate]), Long, String](Year24, 24) {
  ...
  override def partTwo(input: (Map[String, Boolean], Seq[Gate])): String = input match
    case (wires, gates) => {
      ...
      val correctResult = computeCorrectResult(wires)
      val swappedResult = partOne((wires, swappedWires))

      if (correctResult != swappedResult) throw Exception("Swaps do not create the correct result :(")
